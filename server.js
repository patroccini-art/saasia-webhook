...
function callOpenAI(msg, cb) {
  ...
  const req = https.request({hostname:'api.openai.com',path:'/v1/chat/completions',method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+OPENAI_KEY,'Content-Length':Buffer.byteLength(body)}},res=>{let d='';res.on('data',c=>d+=c);res.on('end',()=>{console.log('OpenAI raw response:',d);try{cb(JSON.parse(d).choices[0].message.content);}catch(e){console.log('OpenAI parse error:',e.message);cb('Desculpe, erro interno.');}});});
  req.on('error', e => { console.log('OpenAI request error:', e.message); cb('Erro de conexão.'); });
